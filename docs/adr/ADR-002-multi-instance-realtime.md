...
- `apps/api/src/realtime/phase-timer.service.spec.ts`
  - 자동 phase 전환 deadline은 Redis sorted set에 저장되고 다른 서비스 인스턴스에서도 조회할 수 있다.
...
pnpm --filter api test:phase-timer
```
...
3. 자동 phase timer는 process-local `setTimeout`만으로 운영하지 않고 Redis 기반 deadline poller로 처리한다. 이 결정은 `PhaseTimerService`로 적용한다.
