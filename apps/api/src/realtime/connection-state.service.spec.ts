...
async function cleanupKeys(userId: string, socketIds: string[]) {
  await Promise.all([
    redisService.del(`connection:user:${userId}`),
    ...socketIds.map((socketId) =>
      redisService.del(`connection:socket:${socketId}`),
    ),
  ]);
}
...
test('같은 socket의 markConnected가 기존 roomId를 지우지 않는다', async () => {
  const userId = randomUUID();
  const socketId = randomUUID();

  try {
    await service.markConnected({ userId, socketId });
    await service.setRoom({ userId, socketId, roomId: randomUUID() });

    const reconnected = await service.markConnected({ userId, socketId });

    assert.ok(reconnected.roomId);
    assert.equal(reconnected.socketId, socketId);
    assert.equal((await service.findByUserId(userId))?.roomId, reconnected.roomId);
  } finally {
    await cleanupKeys(userId, [socketId]);
  }
});
...
test('stale disconnect가 newer socket connection을 DISCONNECTED로 덮어쓰지 않는다', async () => {
  const userId = randomUUID();
  const oldSocketId = randomUUID();
  const newSocketId = randomUUID();

  try {
    await service.markConnected({ userId, socketId: oldSocketId });
    await service.markConnected({ userId, socketId: newSocketId });
    ...
    const loaded = await service.findByUserId(userId);

    assert.ok(loaded);
    assert.equal(disconnected.socketId, newSocketId);
    assert.equal(loaded.socketId, newSocketId);
    assert.equal(loaded.status, 'CONNECTED');
  } finally {
    await cleanupKeys(userId, [oldSocketId, newSocketId]);
  }
});

test('stale clearRoom이 newer socket의 roomId를 지우지 않는다', async () => {
  const userId = randomUUID();
  const oldSocketId = randomUUID();
  const newSocketId = randomUUID();
  const oldRoomId = randomUUID();
  const newRoomId = randomUUID();

  try {
    await service.markConnected({ userId, socketId: oldSocketId });
    await service.setRoom({ userId, socketId: oldSocketId, roomId: oldRoomId });
    await service.markConnected({ userId, socketId: newSocketId });
    await service.setRoom({ userId, socketId: newSocketId, roomId: newRoomId });
    ...
    assert.ok(loaded);
    assert.equal(cleared.roomId, newRoomId);
    assert.equal(loaded.socketId, newSocketId);
    assert.equal(loaded.roomId, newRoomId);
  } finally {
    await cleanupKeys(userId, [oldSocketId, newSocketId]);
  }
});
