...
export class ConnectionStateService {
  ...
  async markConnected(input: {
    ...
    const state: RealtimeConnectionState = {
      ...
      roomId:
        current?.socketId === input.socketId ? current.roomId : null,
      status: 'CONNECTED',
      connectedAt: now,
      lastSeenAt: now,
      disconnectedAt: null,
  ...
  async markDisconnected(input: {
    ...
    const base = current ?? this.createBaseState(input);
    const now = new Date().toISOString();
    const state: RealtimeConnectionState = {
      ...base,
      userId: input.userId,
  ...
  async clearRoom(input: {
    ...
    if (current.socketId !== input.socketId) {
      return current;
    }
