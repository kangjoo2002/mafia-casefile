...
class GameSessionDisconnectTestModule {}
...
before(async () => {
  app = await NestFactory.createApplicationContext(
    GameSessionDisconnectTestModule,
    {
      logger: false,
    },
  );
...
test('markPlayerDisconnected()가 player connectionStatus를 DISCONNECTED로 바꾼다', async () => {
  const service = app.get(GameSessionService);
  const gameId = randomUUID();
  const session = await service.startGameSession(createSessionInput(gameId));

  try {
    const disconnectedAt = new Date('2026-05-16T12:05:00.000Z');
    const updated = await service.markPlayerDisconnected({
      gameId,
      userId: 'user-2',
      disconnectedAt,
    });
    ...
    assert.ok(player);
    assert.equal(player?.connectionStatus, 'DISCONNECTED');
    assert.equal(player?.status, 'ALIVE');
    assert.equal(player?.lastSeenAt.toISOString(), disconnectedAt.toISOString());
